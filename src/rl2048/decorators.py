...
class MutateDecorator(Protocol):
    def __call__(
        self, pytree: T, *args: In.args, **kwargs: In.kwargs
    ) -> tuple[T, *Out]:
        ...


def mutates(
    where: str | Callable[[T], PyTree],
    pure_fn: Callable[Concatenate[T, In], tuple[PyTree, *Out]],
) -> Callable[Concatenate[T, In], tuple[T, *Out]]:
